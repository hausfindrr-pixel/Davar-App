...
const SUBHEADING = "text-xs font-semibold uppercase tracking-wide text-stone";
...
/** The whole /admin view — four collapsible sections (Users, Revenue,
 * Peter's Watch, Content), each opening compact with just its title and a
 * one-number preview, so the page doesn't dump every chart/table on
 * screen at once. Purely a rendering of AdminDashboardData (fetched
 * server-side by app/admin/page.tsx, gated by verifyAdminSession) — no
 * client-side fetching; <details>/<summary> (Section.tsx) gives the
 * expand/collapse behavior for free, no client component or JS state
 * needed. Function over polish, per spec: no charting library. */
export function AdminDashboard({ data }: AdminDashboardProps) {
  ...
  return (
    <main className="min-h-dvh bg-ivory px-4 py-6 flex flex-col gap-3 max-w-4xl mx-auto">
      ...
      <Section title="Users" preview={`${formatCompactNumber(users.totalUsers)} signups`} defaultOpen>
        ...
        <span className={SUBHEADING}>Retention — % of each signup week still active 7d later</span>
      ...
      <Section title="Revenue & Subscriptions" preview={formatCompactUsd(revenue.totalRevenueUsd)}>
        ...
        <span className={SUBHEADING}>Expiring soonest</span>
        <ExpiringPremiumTable rows={revenue.expiringPremium.slice(0, 20)} />

        <span className={SUBHEADING}>Recent payments</span>
        <PaymentTable rows={revenue.recentPayments.slice(0, 20)} />

        <span className={SUBHEADING}>Failed / underpaid invoices</span>
      ...
      <Section title="Peter's Watch (AI usage)" preview={formatCompactUsd(watch.estimatedCostUsd)}>
        <div className="grid grid-cols-2 gap-3">
        ...
        <span className={SUBHEADING}>Highest volume (last 7d)</span>
      ...
      <Section title="Content Engagement" preview={`${lessonsByRate.length} lessons`}>
        <div className="grid grid-cols-2 gap-3">
        ...
        <span className={SUBHEADING}>Lesson completion — lowest first</span>
